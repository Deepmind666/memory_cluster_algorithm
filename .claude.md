...
### Review Entry R-048 — Claims and Specification Strengthening for Advisor Review
- **时间戳**: 2026-02-14 21:23:23 +08:00
- **阶段**: 材料撰写阶段（面向师兄审阅）
- **综合结论**: 已按评审意见完成高优先级文本强化：独权收敛、从属承接、术语定义、公式补充、定量实施例补强，并新增一份可直接转发的审阅说明文档。

#### 本轮关键改动
1. `docs/patent_kit/17_CNIPA_权利要求书_正式稿.md`
   - 独权1改为核心链路；
   - 语义抽取细节下放从属；
   - 独立主张覆盖方法/系统/装置/介质；
   - 候选筛选/ANN改为标准限定句式。
2. `docs/patent_kit/16_CNIPA_说明书_正式稿.md`
   - 补背景技术具体对比文献；
   - 增术语定义与三类公式；
   - 增2000规模定量实施例。
3. `docs/patent_kit/21_师兄评审前_改进建议与变更说明.md`
   - 汇总“已改/待代理人处理”边界，便于外部审阅。
4. `docs/patent_kit/00_技术交底书_总览.md`
   - 索引新增 21 号文档入口。

### Review Entry R-049 — CNIPA Figure Compliance Hardening
- **时间戳**: 2026-02-14 21:47:46 +08:00
- **阶段**: 材料撰写阶段（附图强制规则收口）
- **综合结论**: 已把 CNIPA 附图强制风险点（彩色、英文、说明框、门控符号不规范）做了针对性清理，v2 图件可作为当前提交候选。
...
2. 规则对齐：
   - 图内文本全中文（去 `S1/sim/compat/AND/A/B/C` 等英文或公式化文字）
   - 纯黑白配色（仅 `#000/#fff`）
   - 图2补齐开始/结束节点与分支汇合
   - 图5改为 D 形与门
   - 图3关系虚线改为标号 `306/307/308`
3. 文档映射同步：
...
#### 自查与验证
- `rg -n ">[^<]*[A-Za-z][^<]*<" docs/patent_kit/figures -g "fig*_v2*.svg"`：PASS（无英文文本节点）
- `rg -n "#[0-9A-Fa-f]{3,6}" docs/patent_kit/figures -g "fig*_v2*.svg"`：PASS（仅黑白）
- `python -m compileall -q src scripts tests`：PASS
- `python -m unittest discover -s tests -p "test_*.py"`：PASS（108/108）
- `python scripts/run_stage2_guardrail.py --output outputs/stage2_guardrail.json --report docs/eval/stage2_guardrail_report.md --core-stability outputs/core_claim_stability_semi_real_5000_realistic.json --core-stability outputs/core_claim_stability_semi_real_5000_stress.json`：PASS（check_count=14, blocker=0）
- `python scripts/build_patent_evidence_pack.py --output outputs/patent_evidence_pack.json --report "docs/patent_kit/10_区别特征_技术效果_实验映射.md"`：PASS（validation.passed=true）
...
#### Self-Check SC-R049
1. 是否清除附图中的英文/公式化文本：是
2. 是否满足纯黑白线框要求：是
3. 是否消除说明性大段文本框：是
4. 是否同步正式材料中的附图路径：是
5. 是否完成质量门闭环验证：是

### Review Entry R-050 — CNIPA Figure Review Fixes (P1/P2 Closure)
- **时间戳**: 2026-02-15 01:32:14 +08:00
- **阶段**: 材料撰写阶段（评审问题闭环）
- **综合结论**: 已按审查意见关闭 1 个 P1 和 4 个 P2，并顺带收口 2 个 P3。图件逻辑与文档表述已对齐。

#### 评审项闭环
1. `P1-1` 图1输出框重叠：已修复  
   - 文件：`docs/patent_kit/figures/fig1_v2_system_architecture.svg`  
   - 调整：输出箭头 `160/380`，输出框改为 `x=80,w=160` 与 `x=300,w=160`（无重叠）
2. `P2-1` 图4缺少 401-1 到 404 虚线：已修复  
   - 文件：`docs/patent_kit/figures/fig4_v2_conflict_evidence_graph.svg`  
   - 调整：新增 `401-1 -> 404` 虚线连接
3. `P2-2` 标记总表缺少 500：已修复  
   - 文件：`docs/patent_kit/19_CNIPA_附图文字描述稿.md`  
   - 调整：补充 `500：候选簇对输入`
4. `P2-3` 摘要附图指引矛盾：已修复  
   - 文件：`docs/patent_kit/18_CNIPA_说明书摘要_正式稿.md`、`docs/patent_kit/19_CNIPA_附图文字描述稿.md`  
   - 调整：统一为“独立简化版摘要附图（不直接复用图1）”
5. `P2-4` 图1版式描述与实际图不符：已修复  
   - 文件：`docs/patent_kit/19_CNIPA_附图文字描述稿.md`  
   - 调整：改为“自上而下主链路”
...
1. 图4标题统一为“冲突优先级计算”  
   - `docs/patent_kit/figures/fig4_v2_conflict_evidence_graph.svg`
2. 图5/图6 空白收紧  
   - `docs/patent_kit/figures/fig5_v2_dual_channel_gate.svg`（高度 620→500）
   - `docs/patent_kit/figures/fig6_v2_retrieval_backref.svg`（高度 440→340）

#### 自查与验证
- v2 SVG XML 解析：7/7 PASS
- 英文文本扫描：PASS（未发现英文文本节点）
- 黑白色值扫描：PASS（仅 `#000/#fff`）
- `python -m compileall -q src scripts tests`：PASS
- `python -m unittest discover -s tests -p "test_*.py"`：PASS（108/108）
- `python scripts/run_stage2_guardrail.py --output outputs/stage2_guardrail.json --report docs/eval/stage2_guardrail_report.md --core-stability outputs/core_claim_stability_semi_real_5000_realistic.json --core-stability outputs/core_claim_stability_semi_real_5000_stress.json`：PASS（check_count=14, blocker=0）
- `python scripts/build_patent_evidence_pack.py --output outputs/patent_evidence_pack.json --report "docs/patent_kit/10_区别特征_技术效果_实验映射.md"`：PASS（validation.passed=true）
...
1. P1/P2 是否全部闭环：是
2. 图文口径是否统一：是
3. 附图规则（黑白/中文/无说明框）是否保持：是
4. v2 图件结构是否可解析：是
5. 质量门是否通过：是

### Review Entry R-051 — Full Work Inventory and Path Map
- **时间戳**: 2026-02-15 01:32:14 +08:00
- **阶段**: 交接收口（全量梳理）
- **综合结论**: 已形成一份可直接对外评审/交接的全量索引文档，覆盖“做了什么 + 在哪 + 当前状态”三层信息。

#### 新增交付
1. `docs/review/r051_全量工作梳理与文件路径总表.md`

#### 文档覆盖范围
1. 项目目标与当前状态（测试/门禁/证据包）
2. 核心代码模块路径（`src/memory_cluster/*`）
3. 实验与工程脚本路径（`scripts/*`）
4. 测试路径（`tests/*`）
5. 评测与证据路径（`docs/eval/*`, `outputs/*`）
6. 专利材料路径（`docs/patent_kit/00-21` + `figures/*`）
7. 治理与进度路径（`.claude.md`, `WORK_PROGRESS.md`, `docs/FINAL_REPORT.md`, `docs/review/*`）
8. R-049/R-050 修复点到文件的逐项映射

#### Self-Check SC-R051
1. 是否覆盖交接所需主目录：是
2. 是否包含专利提交相关关键文件组：是
3. 是否区分 v2 提交候选附图与历史版：是
4. 是否给出当前状态与后续建议：是
5. 是否可直接用于外部评审沟通：是

### Review Entry R-052 — CNIPA PDF Export Flow + Push Retry Status
- **时间戳**: 2026-02-15 02:56:00 +08:00
- **阶段**: 提交准备（导出流程补齐）
- **综合结论**: 已补齐“附图批量导出 PDF”工具链并验证可用；当前唯一阻塞是外网到 GitHub 的连接问题。

#### 本轮关键动作
1. 推送重试
   - `git push origin main` 多次重试仍失败（`connection reset` / `could not connect to github.com:443`）
2. 新增导出脚本
   - `scripts/export_cnipa_figures_pdf.ps1`
   - 导出对象：7 张 v2 附图
   - 输出目录：`outputs/cnipa_submission/pdf`
   - 产物：7 个 PDF + `EXPORT_MANIFEST.txt`
3. 新增导出说明
   - `docs/patent_kit/22_CNIPA_附图PDF导出说明.md`
4. 总览同步
   - `docs/patent_kit/00_技术交底书_总览.md` 新增 22 号文档

#### 稳定性修正（脚本内部）
1. 输出路径绝对化，规避 Edge 相对路径写入失败
2. 导出前删除旧同名文件，防止假成功
3. 加入导出后等待与文件存在校验
4. 原生进程输出重定向，避免 stderr 噪声导致中断

#### 自查结果
- `powershell -ExecutionPolicy Bypass -File scripts/export_cnipa_figures_pdf.ps1`：PASS
- `outputs/cnipa_submission/pdf/EXPORT_MANIFEST.txt`：PASS（7/7）
- Git 推送：FAIL（网络阻断，非功能回归）

#### Self-Check SC-R052
1. 附图 PDF 是否可批量稳定导出：是
2. 导出清单是否可追踪：是
3. 总览索引是否同步：是
4. 是否引入代码行为风险：否
5. 当前阻塞是否仅剩网络推送：是
